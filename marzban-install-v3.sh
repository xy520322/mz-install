...
collect_input() {
    ...
    echo ""
    echo -e "  ${WHITE}协议:${NC}"
    echo -e "    ${PURPLE}●${NC} VLESS+REALITY (${REALITY_PORT})  ${PURPLE}●${NC} VLESS+WS+TLS (443)"
    echo -e "    ${PURPLE}●${NC} VLESS+gRPC+TLS (443)  ${PURPLE}●${NC} VMess+WS+TLS (443)"
    echo -e "    ${PURPLE}●${NC} Trojan+WS+TLS (443)   ${PURPLE}●${NC} Shadowsocks (${SS_PORT})"
    echo ""
...
step_deps() {
    ...
    apt-get install -y curl wget git unzip jq socat cron dnsutils lsof net-tools ca-certificates gnupg python3 >> "$LOG_FILE" 2>&1 || die "依赖安装失败"
    log "依赖完成"
}
...
#=========================== [6] REALITY 密钥 (5层降级) ===========================#
step_reality_keys() {
    log "[6/13] REALITY 密钥 (5层降级生成)..."
    REALITY_PRIV=""; REALITY_PUB=""

    # 方法1: xray x25519 (ghcr镜像)
    info "尝试方法1: ghcr.io xray x25519..."
    ...
    # 方法2: xray x25519 (dockerhub镜像)
    if [[ -z "$REALITY_PRIV" ]]; then
        info "尝试方法2: teddysun xray..."
        keys=$(docker run --rm teddysun/xray:latest xray x25519 2>/dev/null) || true
        if [[ -n "$keys" ]] && echo "$keys" | grep -qi "private"; then
            REALITY_PRIV=$(echo "$keys" | grep -i "private" | awk '{print $NF}')
            REALITY_PUB=$(echo "$keys" | grep -i "public" | awk '{print $NF}')
        fi
    ...
    # 方法3: 本地安装 xray 二进制生成
    if [[ -z "$REALITY_PRIV" ]]; then
        info "尝试方法3: 下载 xray 二进制..."
        local xray_tmp="/tmp/xray-reality-gen"
        mkdir -p "$xray_tmp"
        local xray_url="https://github.com/XTLS/Xray-core/releases/latest/download/Xray-linux-64.zip"
        if curl -sL "$xray_url" -o "$xray_tmp/xray.zip" 2>/dev/null && unzip -qo "$xray_tmp/xray.zip" -d "$xray_tmp" 2>/dev/null; then
            chmod +x "$xray_tmp/xray" 2>/dev/null
            keys=$("$xray_tmp/xray" x25519 2>/dev/null) || true
            if [[ -n "$keys" ]] && echo "$keys" | grep -qi "private"; then
                REALITY_PRIV=$(echo "$keys" | grep -i "private" | awk '{print $NF}')
                REALITY_PUB=$(echo "$keys" | grep -i "public" | awk '{print $NF}')
            fi
        fi
        rm -rf "$xray_tmp" 2>/dev/null
    fi

    # 方法4: Python cryptography 库
    if [[ -z "$REALITY_PRIV" ]]; then
        info "尝试方法4: Python cryptography..."
        keys=$(python3 -c "
try:
    from cryptography.hazmat.primitives.asymmetric.x25519 import X25519PrivateKey
    from base64 import urlsafe_b64encode
    k = X25519PrivateKey.generate()
    priv = urlsafe_b64encode(k.private_bytes_raw()).decode().rstrip('=')
    pub = urlsafe_b64encode(k.public_key().public_bytes_raw()).decode().rstrip('=')
...
" 2>/dev/null) || true
        if [[ -n "$keys" ]]; then
            REALITY_PRIV=$(echo "$keys" | grep "Private:" | cut -d: -f2)
            REALITY_PUB=$(echo "$keys" | grep "Public:" | cut -d: -f2)
        fi
    fi

    # 方法5: pip install cryptography 再生成
    if [[ -z "$REALITY_PRIV" ]]; then
        info "尝试方法5: pip install cryptography..."
        pip3 install cryptography --break-system-packages -q >> "$LOG_FILE" 2>&1 || pip3 install cryptography -q >> "$LOG_FILE" 2>&1 || true
        keys=$(python3 -c "
from cryptography.hazmat.primitives.asymmetric.x25519 import X25519PrivateKey
from base64 import urlsafe_b64encode
k = X25519PrivateKey.generate()
...
" 2>/dev/null) || true
    ...
    # 最终检查
    if [[ -n "$REALITY_PRIV" && -n "$REALITY_PUB" && "$REALITY_PRIV" != "YOUR"* ]]; then
        log "REALITY 密钥生成成功"
        REALITY_PLACEHOLDER=false
    else
        die "所有方法均无法生成 REALITY 密钥! 请检查网络或手动安装 python3-cryptography"
    fi
}
...
B
    ...
    echo ""
    echo -e "  ${WHITE}协议:${NC}"
    echo -e "    ${PURPLE}●${NC} VLESS+REALITY → ${CYAN}${REALITY_PORT}${NC}  ${PURPLE}●${NC} VLESS+WS → ${CYAN}443${NC}"
    echo -e "    ${PURPLE}●${NC} VLESS+gRPC → ${CYAN}443${NC}     ${PURPLE}●${NC} VMess+WS → ${CYAN}443${NC}"
    echo -e "    ${PURPLE}●${NC} Trojan+WS → ${CYAN}443${NC}     ${PURPLE}●${NC} Shadowsocks → ${CYAN}${SS_PORT}${NC}"
    echo ""
...
main() {
    ...
    step_nginx_config; step_firewall; step_start; step_finalize
}
